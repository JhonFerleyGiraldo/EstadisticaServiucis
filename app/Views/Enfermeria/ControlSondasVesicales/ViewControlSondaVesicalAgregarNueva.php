...
<head>
  ...
  <title>Estadística | Nueva Sonda Vesical</title>
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Nueva Sonda Vesical</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
              <form role="form">
            ...
            <div class="row">
              ...
              <div class="col-md-2">
                <input class="form-control" type="date" value="<?=date('Y-m-d')?>" max="<?=date('Y-m-d')?>" id="fechaInsercion">
							...
							<div class="col-md-2">
								<label for="numSonda">Num. Sonda</label>
              </div>
              <div class="col-md-2">
                <input type="text" value="8" onKeyPress="return soloNumeros(event)"  class="form-control" id="numSonda" maxlength="2" >
              </div>
            </div> 
          </div>
              ...
              <div class="row">
                ...
                <div class="card-footer">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <button id="botonAgregarSonda" type="button" onclick="AgregarNuevaSonda();" class="btn btn-primary"><img src="<?=BASEURL?>app/img/agregar.png"></img> Agregar</button>
                            </div> 
                        </div> 
                </div>
              </form>
...
function ConsultarDatosPaciente(documento,numIngreso){
  ...
  $.ajax({
            ...
            url: '../ControlSondasVesicales/GetConsultarDatosPaciente',//llamado a metodo
...
function AgregarNuevaSonda(){
    ...
    if(ValidarCampos()){
      ...
      if(lugarProcedimiento=='Interno'){
        idCtrlSondas=GuardarNuevaSonda(codigoIngresoTabla,fechaInsercion,numeroSonda,
                                            lugarProcedimiento,codenfermero);
      }else{
        //Si es un procedimiento externo el enfermero  va vacio
        idCtrlSondas=GuardarNuevaSonda(codigoIngresoTabla,fechaInsercion,numeroSonda,
                                            lugarProcedimiento,null);
      }

      if(fechaRetiro!=''){
        GuardarRetiroSonda(idCtrlSondas,codigoIngresoTabla,fechaRetiro,motivoRetiro,cultivo,reporte);
      }
...
function GuardarNuevaSonda(codigoIngreso,fechaInsercion,numSonda,lugarProcedimiento,enfermero){
  var idCtrlSonda;
  $.ajax({
        type: "POST",
        dataType: 'json',
        async:false,//para que no sea asincrono
        url: '../ControlSondasVesicales/SetAgregarNuevaSonda',//llamado a metodo
        data: { "codIngreso":codigoIngreso,"fechaInsercion":fechaInsercion,"numSonda":numSonda,"lugarProcedimiento":lugarProcedimiento,"enfermero":enfermero}, //parametros
        success: function(response)
        {   

            idCtrlSonda=response;
          //console.log(response);
        },
        error: function (error) {
          //mensajes de error
            $(document).Toasts('create', {
                ...
                title: 'Validación',
                subtitle: 'Error',
                body: 'Error agregando nueva sonda (error llamando consulta).',
                icon:"fas fa-exclamation-circle",
                autohide:true,
                delay:5000
            });  
...
function GuardarRetiroSonda(idSonda,codigoIngreso,fechaRetiro,motivo,cultivo,reporte){

  $.ajax({
        type: "POST",
        dataType: 'json',
        async:false,//para que no sea asincrono
        url: '../ControlSondasVesicales/SetRetirarSonda',//llamado a metodo
        data: { "idSonda":idSonda,"codIngreso":codigoIngreso,"fechaRetiro":fechaRetiro,"motivo":motivo,"cultivo":cultivo,"reporte":reporte}, //parametros
        success: function(response)
        {   
          if(response==1){
              $(document).Toasts('create', {
                ...
                title: 'Validación',
                subtitle: 'Aviso',
                body: 'Sonda retirada correctamente.',
              ...
              $(document).Toasts('create', {
                  ...
                  title: 'Validación',
                  subtitle: 'Error',
                  body: 'Error retirando la sonda (error llamando consulta).',
                  icon:"fas fa-exclamation-circle",
                  autohide:true,
                  delay:5000
              }); 
...
*/
    ...
    if($("#numSonda").val()==''){
        $(document).Toasts('create', {
                    ...
                    title: 'Validación',
                    subtitle: 'Mensaje',
                    body: 'Se debe ingresar número de sonda.',
                    icon:"fas fa-exclamation-circle",
                    autohide:true,
                    delay:5000
        });
    ...
    if(numeroSonda%2){
        $(document).Toasts('create', {
            ...
            title: 'Validación',
            subtitle: 'Mensaje',
            body: 'El número de sonda debe ser par.',
            icon:"fas fa-exclamation-circle",
            autohide:true,
            delay:5000
        });
    ...
    if(numeroSonda<8 || numeroSonda>26){
        $(document).Toasts('create', {
            class: 'bg-warning', 
            title: 'Validación',
            subtitle: 'Mensaje',
            body: 'El calibre de la sonda va desde 8 hasta 26.',
            icon:"fas fa-exclamation-circle",
            autohide:true,
            delay:5000
        });

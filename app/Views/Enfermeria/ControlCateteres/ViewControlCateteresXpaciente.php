...
<head>
  ...
  <title>Estadística | Catéter por Paciente</title>
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">Control Catétetes</h1>
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-2">
            <?php if($_SESSION["perfilUsuario"] == "ENFERMERIA" || $_SESSION["perfilUsuario"]=="ADMINISTRADOR"): ?>
              <a href="<?=BASEURL?>ControlCateteres/GetVistaAgregarCateter&documento=<?=$documento;?>&numIngreso=<?=$numIngreso;?>"><button type="button" id="btnNuevoCateter" class="btn btn-block btn-primary"><img src="<?=BASEURL?>app/img/agregar.png"></img> Nuevo Catéter</button></a>
            <?php endif; ?>
          </div>
    ...
    <div class="card">
            <div class="card-header">
              <h3 class="card-title">Listado catéteres del paciente</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
...
$(document).ready(function(){
    ...
    if($("#documento").val()!=0 && $("#numIng").val()!=0){
        ...
        var datosCateter=ConsultarCateteresPorIngreso(codigoIngresoTabla);
        //console.log(datosCateter);

        var j=1;
        $(datosCateter).each(function(i, v){

            var boton="";
            //validamos con la fecha de retirp en que estado se encuentra el cateter
            if(v.fechaRetiro==null){
              boton="<a href='<?=BASEURL?>ControlCateteres/GetVistaRetirarCateter&codCateter=" + v.codigo + "&codIngreso=" + v.codIngreso + "'><button type='button' class='btn btn-block btn-primary'>Abrir</button></a>";
            }else{
              boton="<a href='<?=BASEURL?>ControlCateteres/GetVistaRetirarCateter&codCateter=" + v.codigo + "&codIngreso=" + v.codIngreso + "'><button type='button' class='btn btn-block btn-success'>Consultar</button></a>";
            }
            ...
            $('#tablaCateteres').append('<tr><td>' + v.insercion + '</td><td>' + v.tipoCateter + 
            '</td><td>' + v.ubicacion + '</td><td>' + v.lugar +  '</td><td>' + v.medico +   
            '</td><td>' + v.enfermero + '</td><td>' + boton + '</td></tr>');
            j++;
        });
...
function ConsultarCateteresPorIngreso(codigoIngreso){
...
$.ajax({
          ...
          url: '../ControlCateteres/GetConsultarCateteresPorIngreso',//llamado a metodo

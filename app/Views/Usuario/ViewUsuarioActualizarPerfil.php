                ...
                <div class="card-body">
					...
					<div class="form-group">
                        <div class="row">
                            ...
                            <div class="col-md-2">
                                <select class="form-control select2" id="perfil" style="width: 100%;" disabled>
                                    <option ></option>
                            ...
                            <div class="col-md-2">
                                <select class="form-control select2" id="estado" style="width: 100%;" disabled>
                                    <option value="" ></option>
...
function ActualizarUsuario(){
    ...
    if(ValidarCampos()){
        ...
        //validamos si se cambio la clave en los campos de clave
        if(clave!=''){
          //Actualizamos el usuario
          guardado=GuardarActualizacionUsuario(codUsuario,clave);
        }
      ...
      if($("#clave").val()!=''){
        ...
        if(clave.length>=8){
          ...
          $(document).Toasts('create', {
                      ...
                      title: 'Validación',
                      subtitle: 'Mensaje',
                      body: 'La clave debe ser mínimo de 8 caracteres.',
                      icon:"fas fa-exclamation-circle",
                      autohide:true,
                      delay:5000
          });
...
function GuardarActualizacionUsuario(codUsu,pass){
    var resultado;
    $.ajax({
        type: "POST",
        dataType: 'json',
        async:false,//para que no sea asincrono
        url: '../Usuario/SetActualizarClaveUsuario',//llamado a metodo
        data: {"codUsu":codUsu,"pass":pass}, //parametros
        success: function(response)
        {   
    ...
    $.ajax({
        ...
        {   
            ...
            $(response).each(function(i, v){
                ...
                if(v.estado=='S'){
                $('#tablaSedes').append('<tr><td>' + v.codigoSede + '</td><td>' +  v.sede + '</td><td class="check"><input type="checkbox" checked disabled/></td></tr>');
                }else{
                $('#tablaSedes').append('<tr><td>' + v.codigoSede + '</td><td>' +  v.sede + '</td><td class="check"><input type="checkbox" disabled/></td></tr>');   
                }
